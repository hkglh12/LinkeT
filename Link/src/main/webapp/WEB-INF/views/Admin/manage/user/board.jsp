	...
	<div class="dashboard mt curr">
      	<div class="large-12 forum-category rounded top">
        	...
        	<c:if test="${category eq 'all'}">
          	...
          	<c:if test="${category eq 'normal'}">
          		<label>정상사용자</label>
          	</c:if>
          	<c:if test="${category eq 'banout'}">
      ...
      <div class="toggleview">
      		<div class="large-12 forum-head">
         		...
         		<div class="large-5 column ltpad">
          		...
          		<div class="large-2 column ltpad">
            		작성 게시글 개수
          		</div>
          		<div class="large-2 column ltpad">
            		작성 댓글 개수
         		...
         		<c:forEach items="${list}" var="user" varStatus = "number">
        			<div class="large-12 forum-topic cal">
          				...
          				<div class="large-15 small-4 column lpad">
       				    ...
       				    <div class="large-5 column ltpad">
       				    	<c:if test="${user.usrKickedDate ne null}">
            					<span class="center" id="counts">${user.usrBannedId}</span>
            				</c:if>
            				<c:if test="${user.usrKickedDate eq null}">
            					<span class="center" id="counts"> - </span>
            				</c:if>
          				</div>
          				<div class="large-5 column ltpad">
          					<c:if test="${user.usrLevel eq 99}">
          						<label>관리자</label>
          					</c:if>
       				    	<c:if test="${user.usrKickedDate ne null or user.usrOutDate ne null}">
            				</c:if>
            				<c:if test="${(user.usrKickedDate eq null and user.usrOutDate eq null) and (user.usrLevel ne 99)}">
