$(document).ready(function(){
	...
	// 변수 선언을 다르게 했기에, 애니메이션 삽입 하지 아니함.
...
function bann(user_id){
	...
	if (result == true){
		var frm = $("<form>").attr("action","/Link/admin/manage/user/ban").attr("method","post").appendTo($("body"));
		$("<input>").attr("type","hidden").attr("name","target_list").attr("value", user_id).appendTo(frm);
		$("<input>").attr("type","hidden").attr("name","user_id").attr("value",user_id).appendTo(frm);
