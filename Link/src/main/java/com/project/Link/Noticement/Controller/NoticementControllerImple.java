...
public class NoticementControllerImple implements NoticementController {
	...
	public String UpdateNoticement(Model model, MultipartHttpServletRequest mpRequest, HttpSession session, RedirectAttributes redirectAttr) throws Exception {
		...
		boolean result = nService.updateNoticement(usrId, serial, title, contents, previousFileCodes, deleteFileCodes, uFileList);
			/*
			 * redirectAttr.addFlashAttribute("usrId", sr.get("usrId"));
			 * redirectAttr.addFlashAttribute("isAdmin", sr.get("isAdmin"));
			 */
	...
	public String DeleteNoticement(Model model, HttpServletRequest request, HttpSession session, RedirectAttributes redirectAttr) {
		...
		int targetSerial = Integer.valueOf(request.getParameter("n_serial"));
		String usrId = (String)session.getAttribute("usrId");
		boolean result = nService.deleteNoticement(targetSerial, usrId);
		/*
		 * redirectAttr.addFlashAttribute("usrId", sr.get("usrId"));
		 * redirectAttr.addFlashAttribute("isAdmin", sr.get("isAdmin"));
		 */
